...
<div style="margin-right:5px; float:right;" id="sticky">
 <a href="#" class="mbutton blog_save_new <?php
   if (isset($settings_install) && is_array($settings_install) && count($settings_install) > 0 && isset($settings_install['ascp_install_4']) && !$settings_install['ascp_install_4']) {
   ?> asc_blinking <?php
   }
...
function delayer(){
  window.location = 'index.php?route=agoo/blog/widgets&tab='+tab+'&<?php echo $token_name; ?>=<?php echo $token; ?>';
}


</script>
<?php  if (isset($ascp_widgets) && is_array($ascp_widgets) && count($ascp_widgets) > 0) { ?>
...
<div class="input-group sc_table_row">
    ...
    this_block_html = $('#mytabs_add').html();
 	$.ajax({
					url: 'index.php?route=agoo/blog/ajax_list&<?php echo $token_name; ?>=<?php echo $token; ?>',
					type: 'post',
					async: true,
					...
					beforeSend: function()
					{
                      $('#mytabs_add').html('<?php echo $language->get('text_loading_main'); ?>');
					},
...
<?php } ?>
    ...
    finishCallback = function(){
		$.ajax({
		                url: 'index.php?route=agoo/blog/ascp_widgets_save&<?php echo $token_name; ?>=<?php echo $token; ?>',
		                type: 'post',
							dataType: 'html',
							async: true,
							success: function(html) {
...
<?php } ?>
          ...
          //console.log($(kf).html());
            ...
            $.ajax({
                url: 'index.php?route=agoo/blog/ascp_widgets_save&num='+num+'&<?php echo $token_name; ?>=<?php echo $token; ?>',
                type: 'post',
					data: $(kf).serialize(),
					dataType: 'html',
					async: true,
...
<?php  if (isset($ascp_widgets) && is_array($ascp_widgets) && count($ascp_widgets) > 0) {
 ...
 foreach ($ascp_widgets as $num =>$list) {
	...
	if (isset($list['title_list_latest'][ $config_language_id]) &&  $list['title_list_latest'][ $config_language_id]!='')
	{
    	$title= str_replace($zamena,"", $list['title_list_latest'][ $config_language_id]);
	}
	else
	{
		$title="Widget-".$num;
...
<?php } ?>
		...
		$.ajax({
					...
					error: function() {
                    	alert('Error loading widget. Please reload!');
					},
					success: function(html) {
						if (html) {
							...
							alert('May be error loading widget. Please reload!')
						}
	...
	$('.template').each(function() {
		...
		$(e).autocomplete({
			'source': function(request, response) {
				$.ajax({
					url: 'index.php?route=agoo/blog/autocomplete_template&path='+path+'&<?php echo $token_name; ?>=<?php echo $token; ?>',
					dataType: 'json',
