...
<div class="modal fade" id="dialog-set-tooltip">
  <div class="modal-dialog">
	...
	<script type="text/javascript" src="<?php echo $HTTP_URL; ?>view/stylesheet/mf/js/ajax-bootstrap-select.js"></script>
		...
		<div class="container-fluid">
			...
			<script type="text/javascript">
				...
				function mf_init_items_table( idx, type, action ) {
					...
					function reload( url, $progress, id, type ) {
						...
						$.get( mf_fix_url( url ), {}, function( response ){
							...
							if( typeof id != 'undefined' ) {
								$('#' + idx + '_mf-' + type + '-content .selectpicker_mf[id="find-'+type+'-by-name'+idx+'"]').find('option[value="' + id + '"]').removeAttr('data-subtext').removeAttr('disabled').removeData('subtext');
								$('#' + idx + '_mf-' + type + '-content .selectpicker_mf[id="find-'+type+'-by-name'+idx+'"]').mf_selectpicker('refresh');
							}
						});
					}
					...
					$('#' + idx + '_mf-' + type + '-content .selectpicker_mf').mf_selectpicker().each(function(){
						var action_find_values = <?php echo isset( $action_find_values ) ? "'" . $action_find_values . "'" : 'null'; ?>,
							type = $(this).attr('data-type');
						
						if( action_find_values == null ) return;
						
						if( type != 'attribs' && type != 'options' && type != 'filters' ) return;
						
						if( $(this).find('option').length < 999 ) return;
						
						$(this).mf_ajaxSelectPicker({
							ajax: {
								url: mf_fix_url( action_find_values ),
								data: function () {
									var params = {
										q: '{{{q}}}',
										type: type,
							...
							preprocessData: function(data){
								...
								$('#' + idx + '_mf-' + type + '-table tr[data-id]').each(function(){
								...
								for( var i = 0; i < data.length; i++ ) {
									...
									if( typeof data[i].disabled != 'undefined' ) {
										item.disabled = data[i].disabled;
										
										delete item.data.disabled;
									} else if( ids.indexOf( data[i].id ) > -1 ) {
										item.disabled = true;
										item.data.subtext = '<?php echo addslashes( $text_this_item_is_already_added ); ?>';
									}
				...
				<?php } ?>
					<?php if( $tab_active != 'license' && $tab_active != 'seo_base_settings' ) { ?>
						<ul class="nav nav-tabs">
							...
							<li<?php if( $tab_active == 'seo' || $tab_active == 'aliases' ) { ?> class="active"<?php } ?>><a href="<?php echo $tab_seo_link; ?>"><i class="glyphicon glyphicon-link"></i> <?php echo $tab_seo; ?></a></li>
