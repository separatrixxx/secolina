...
var MFP = {
	...
	init: function( modules, firstModule ) {
		...
		if( MF_CAN_LOAD_TABS ) {
			...
			var temp_progress = self.createProgress( '<?php echo $text_loading_please_wait; ?>' ),
				temp_interval = setInterval(function(){
					if( ! MF_CAN_LOAD_TABS ) {
						return;
					}

					temp_progress.remove();
		...
		var self	= this,
			...
			$tc		= jQuery('<div class="tab-content">')
				...
				.append(jQuery('<div id="tab-' + self._row + '-settings" class="tab-pane active">')
					.append(jQuery('<table class="table table-tbody">')
						.append(jQuery('<tr>')
							.append('<td width="200"><?php echo $entry_layout; ?></td>')
		...
		$module.find('[data-name="widget_with_swipe"]')
			.append( self.createField( 'radio_group', '[widget_with_swipe]', typeof data['widget_with_swipe'] == 'undefined' ? '1' : data['widget_with_swipe'], {
				...
				'items' : [
					[ '1', '<?php echo $text_yes; ?>' ],
					[ '0', '<?php echo $text_no; ?>' ],
					[ 'open', '<?php echo $text_only_to_open; ?>' ],
					[ 'close', '<?php echo $text_only_to_close; ?>' ]
				]}})
			)
		...
		$module.find('input[name="category-name"]:not([autocomplete])').each(function(){
			...
			jQuery(this).attr('autocomplete','1').autocomplete({
				...
				source: function(request, response) {
					if( $self.val() ) {
						jQuery.ajax({
							url: 'index.php?route=extension/module/mega_filter/category_autocomplete&user_token=<?php echo $token; ?>&filter_name=' +  encodeURIComponent($self.val())+MF_AJAX_PARAMS,
							dataType: 'json',
		...
		$module.find('input[name="category-hide-name"]:not([autocomplete])').each(function(){
			...
			jQuery(this).attr('autocomplete','1').autocomplete({
				...
				source: function(request, response) {
					if( $self.val() ) {
						jQuery.ajax({
							url: 'index.php?route=catalog/category/autocomplete&user_token=<?php echo $token; ?>&filter_name=' +  encodeURIComponent($self.val()) + MF_AJAX_PARAMS,
							dataType: 'json',
							success: function(json) {		
								response(jQuery.map(json, function(item) {
									return {
	...
	createCategories: function( data, _data ) {
		...
		function add( type, data ) {
			...
			if( type == 'related' ) {
				...
				// generate normal links instead of AJAX request
				/*tbody.append(jQuery('<tr>')
					.append('<td width="200"><?php echo $entry_generate_normal_links_instead_of_ajax_request; ?></td>')
					.append(jQuery('<td>')
						.append( self.render_btn_group( name + '[generate_normal_links_instead_of_ajax_request]', typeof data == 'undefined' || data.generate_normal_links_instead_of_ajax_request != '1' ? false : true ) )
					)
				);*/
			}
