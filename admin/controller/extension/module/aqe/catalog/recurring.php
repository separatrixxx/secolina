...
class ControllerExtensionModuleAqeCatalogRecurring extends Controller {
	...
	public function quick_update() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateUpdateData($this->request->post)) {
			...
			foreach ((array)$id as $_id) {
				$result = $this->model_extension_module_aqe_catalog_recurring->quickEditRecurring($_id, $column, $value, $lang_id, $this->request->post);
				if ($result !== false) {
					$_results[$_id] = $result;
					$results['done'][] = $_id;
				} else {
	...
	protected function validateUpdateData(&$data) {
		...
		if (in_array($column, array('sort_order', 'duration', 'cycle', 'trial_duration', 'trial_cycle', 'price', 'trial_price')) && strpos(trim($data['new']), "#") === 0 && preg_match('/^#\s*(?P<operation>[+-\/\*])\s*(?P<operand>-?\d+\.?\d*)(?P<percent>%)?$/', trim($data['new'])) !== 1) {
			$errors = true;
