<?php
class ControllerExtensionModuleAqeCatalogDownload extends Controller {
	...
	public function __construct($registry) {
		...
		if (!$this->config->get('module_admin_quick_edit_installed') || !$this->config->get('module_admin_quick_edit_status')) {
			$this->response->redirect($this->url->link('catalog/download', 'user_token=' . $this->session->data['user_token'], true));
		}
	}

	public function index() {
		$this->load->model('catalog/download');
		$this->load->model('extension/module/aqe/catalog/download');

		$this->load->language('catalog/download');
		$this->load->language('extension/module/aqe/catalog/general');
		$this->load->language('extension/module/aqe/catalog/download');
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			...
			foreach($this->config->get('module_admin_quick_edit_catalog_downloads') as $column => $attr) {
			...
			if ((int)$this->config->get('module_admin_quick_edit_override_menu_entry')) {
				$this->response->redirect($this->url->link('catalog/download', 'user_token=' . $this->session->data['user_token'] . $url . '&aqer=1', true));
			} else {
				$this->response->redirect($this->url->link('extension/module/admin_quick_edit/catalog__download__', 'user_token=' . $this->session->data['user_token'] . $url, true));
			}
		}
	...
	protected function getList() {
		...
		if (isset($this->request->get['sort'])) {
			...
			$sort = $this->config->get('module_admin_quick_edit_catalog_downloads_default_sort');
		}

		if (isset($this->request->get['order'])) {
			$order = $this->request->get['order'];
		} else {
			$order = $this->config->get('module_admin_quick_edit_catalog_downloads_default_order');
		}
		...
		$data['breadcrumbs'][] = array(
			...
			'href'      => (int)$this->config->get('module_admin_quick_edit_override_menu_entry') ? $this->url->link('catalog/download', 'user_token=' . $this->session->data['user_token'] . $url . '&aqer=1', true) : $this->url->link('extension/module/admin_quick_edit/catalog__download__', 'user_token=' . $this->session->data['user_token'] . $url, true),
			'active'    => true
		);

		$data['add'] = $this->url->link('catalog/download/add', 'user_token=' . $this->session->data['user_token'] . $url . '&aqer=1', true);
		$data['delete'] = $this->url->link('catalog/download/delete', 'user_token=' . $this->session->data['user_token'] . $url . '&aqer=1', true);
		...
		$columns = $this->config->get('module_admin_quick_edit_catalog_downloads');
		...
		$results = $this->model_extension_module_aqe_catalog_download->getDownloads($filter_data);

		$download_total = $this->model_extension_module_aqe_catalog_download->getTotalDownloads();
				...
				switch ($action) {
					case 'edit':
						$_buttons[] = array(
							...
							'url'   => html_entity_decode($this->url->link('catalog/download/edit', '&download_id=' . $result['download_id'] . '&user_token=' . $this->session->data['user_token'] . $url . '&aqer=1', true), ENT_QUOTES, 'UTF-8'),
							'icon'  => 'pencil',
							'name'  => null,
		...
		$data['update_url'] = html_entity_decode($this->url->link('extension/module/admin_quick_edit/catalog__download__quick_update', 'user_token=' . $this->session->data['user_token'], true));
		$data['load_popup_url'] = html_entity_decode($this->url->link('extension/module/admin_quick_edit/catalog__download__load_popup', 'user_token=' . $this->session->data['user_token'], true));
		...
		foreach ($this->config->get('module_admin_quick_edit_catalog_downloads') as $column => $attr) {
			if ((int)$this->config->get('module_admin_quick_edit_override_menu_entry')) {
				$data['sorts'][$column] = $this->url->link('catalog/download', 'user_token=' . $this->session->data['user_token'] . '&sort=' . $attr['sort'] . $url . '&aqer=1', true);
			} else {
				$data['sorts'][$column] = $this->url->link('extension/module/admin_quick_edit/catalog__download__', 'user_token=' . $this->session->data['user_token'] . '&sort=' . $attr['sort'] . $url, true);
			}
		}
		...
		if ((int)$this->config->get('module_admin_quick_edit_override_menu_entry')) {
			$pagination->url = $this->url->link('catalog/download', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}' . '&aqer=1', true);
		} else {
			$pagination->url = $this->url->link('extension/module/admin_quick_edit/catalog__download__', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}', true);
		}
		...
		$template = 'extension/module/aqe/catalog/download_list';
	...
	public function quick_update() {
		...
		if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateUpdateData($this->request->post)) {
			...
			foreach ((array)$id as $_id) {
				$result = $this->model_extension_module_aqe_catalog_download->quickEditDownload($_id, $column, $value, $lang_id, $this->request->post);
				if ($result !== false) {
					$_results[$_id] = $result;
					$results['done'][] = $_id;
				} else {
	...
	private function validatePermissions() {
		if (!$this->user->hasPermission('modify', 'catalog/download') || !$this->user->hasPermission('modify', 'extension/module/admin_quick_edit')) {
