...
$('".$this->language->get('text_anchor_templates_selector')."').html(data);",
		...
		if (isset($this->data['id']) && !isset($this->data['ascp_widgets'][$this->data['id']]['bots'])) {
			$this->data['ascp_widgets'][$this->data['id']]['bots'] = 
			'Googlebot' . PHP_EOL .
			'bingbot' . PHP_EOL .			
			'msnbot' . PHP_EOL .			
	...
	public function editTheme($adapter_theme, $post) {
		...
		if (isset($post['not_replace']) && $post['not_replace'] != '') {
			$html_new = $success_data;
