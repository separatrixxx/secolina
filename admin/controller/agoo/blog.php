...
class ControllerAgooBlog extends Controller
{
	...
	protected $theme_config_template;
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$css_dir = DIR_CATALOG . 'view/theme/'. $this->theme_config_template . '/stylesheet/';
        ...
        if (!is_array($this->data['ascp_settings'])) {
        	$this->data['ascp_settings'] = Array();
        }
        $file_theme_settings =  DIR_CATALOG . "view/theme/" . $this->theme_config_template . "/template/agootemplates/settings.php";
        ...
        if (isset($this->data['ascp_settings'])) {
...
<!-- AddThis Button END -->';
		...
		if (SC_VERSION < 20) {
			...
			if ($this->theme_config_template == 'sstore') {
...
/***************************************/
	public function autocomplete_template() {
		...
		if (SC_VERSION > 21 && !$this->theme_config_template || $this->theme_config_template == '') {
		...
		if (isset($this->request->get['path'])) {
			...
			$this->data['widgets_full_path_theme'] = $this->msearchdir(DIR_CATALOG . "view/theme/" . $this->theme_config_template . "/template/agootemplates/" . $path);
			...
			foreach ($this->data['widgets_full_path'] as $widget_full_path) {
				...
				$ename = str_replace(DIR_CATALOG . "view/theme/" . $this->theme_config_template . "/template/agootemplates/" . $path . "/", '', $dname);
			...
			foreach ($this->data['widgets'] as $in => $result) {
				if (isset($result['name'])) {
					$json[] = array(
						...
						'theme' => strip_tags(html_entity_decode($this->theme_config_template, ENT_QUOTES, 'UTF-8')),
					);
				}
			}
		}
...
KEY( `date` )
		...
		$r = $this->db->query("SHOW INDEX FROM  `" . DB_PREFIX . "order_product`");

		if ($r->num_rows == 1) {
			foreach ($r->rows as $trow) {
				if ($trow['Column_name'] != 'product_id') {
...
EOF;
	 ...
	 public function __construct($registry) {
		...
		if ($this->config->get('config_template') && !is_array($this->config->get('config_template')) && $this->config->get('config_template') != '') {
			$this->theme_config_template = $this->config->get('config_template');
		} else {
        	$this->theme_config_template = 'default';
		}
		if (SC_VERSION > 21 && !$this->theme_config_template || $this->theme_config_template == '') {
			$directory = $this->config->get('config_theme').'_directory';
			if ($this->config->get($directory) == '') {
