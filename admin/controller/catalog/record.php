...
// Commercial license
if (!class_exists('ControllerCatalogRecord', false)) {
class ControllerCatalogRecord extends Controller
{
	...
	public function insert() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			foreach ($this->request->post['record_description'] as $language_id => $value) {
				if ($value['sdescription'] == '&lt;p&gt;&lt;br&gt;&lt;/p&gt;') {
					$this->request->post['record_description'][$language_id]['sdescription'] = '';
				}
				if ($value['description'] == '&lt;p&gt;&lt;br&gt;&lt;/p&gt;') {
					$this->request->post['record_description'][$language_id]['description'] = '';
				}
			}
			...
			if (!isset($this->request->post['button_apply']) || !$this->request->post['button_apply']) {
				if (SC_VERSION < 20) {
					$this->redirect($this->url->link('catalog/record', $this->data['token_name'].'=' . $this->session->data[$this->data['token_name']]));
				} else {
					$this->response->redirect($this->url->link('catalog/record', $this->data['token_name'].'=' . $this->session->data[$this->data['token_name']]));
				}
			}
		}
		$this->getForm();
	...
	private function getList() {
		...
		$this->data['url_switchstatus']   = str_replace('&amp;','&',$this->url->link('catalog/record/switchstatus', $this->data['token_name'].'=' . $this->session->data[$this->data['token_name']]));
		...
		$this->data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('catalog/record', $this->data['token_name'].'=' . $this->session->data[$this->data['token_name']] . $url),
			'separator' => ' :: '
		);
		$this->data['insert']        = $this->url->link('catalog/record/insert', $this->data['token_name'].'=' . $this->session->data[$this->data['token_name']] . $url);
		$this->data['copy']          = $this->url->link('catalog/record/copy', $this->data['token_name'].'=' . $this->session->data[$this->data['token_name']] . $url);
		$this->data['delete']        = $this->url->link('catalog/record/delete', $this->data['token_name'].'=' . $this->session->data[$this->data['token_name']] . $url);
		...
		if (isset($this->request->get['limit'])) {
			$config_limit_admin = (int)$this->request->get['limit'];
		}
		...
		foreach ($results as $result) {
			...
			$action[] = array(
				...
				'href' => $this->url->link('catalog/record/update', $this->data['token_name'].'=' . $this->session->data[$this->data['token_name']] . '&record_id=' . $result['record_id'] . $url)
			);
			...
			$this->data['records'][] = array(
				...
				'url' => HTTPS_CATALOG . 'index.php?route=record/record&record_id=' . $result['record_id'],
				'name' => $result['name'],
				'blog' => $blog_main['name'],
		...
		$this->data['sort_name']     = $this->url->link('catalog/record', $this->data['token_name'].'=' . $this->session->data[$this->data['token_name']] . '&sort=pd.name' . $url);
		$this->data['sort_blog']     = $this->url->link('catalog/record', $this->data['token_name'].'=' . $this->session->data[$this->data['token_name']] . '&sort=blog_name' . $url);
		$this->data['sort_price']    = $this->url->link('catalog/record', $this->data['token_name'].'=' . $this->session->data[$this->data['token_name']] . '&sort=p.price' . $url);
		$this->data['sort_quantity'] = $this->url->link('catalog/record', $this->data['token_name'].'=' . $this->session->data[$this->data['token_name']] . '&sort=p.quantity' . $url);
		$this->data['sort_status']   = $this->url->link('catalog/record', $this->data['token_name'].'=' . $this->session->data[$this->data['token_name']] . '&sort=p.status' . $url);
		$this->data['sort_date']     = $this->url->link('catalog/record', $this->data['token_name'].'=' . $this->session->data[$this->data['token_name']] . '&sort=p.date_available' . $url);
		$this->data['sort_order']    = $this->url->link('catalog/record', $this->data['token_name'].'=' . $this->session->data[$this->data['token_name']] . '&sort=p.sort_order' . $url);
		...
		$pagination->url               = $this->url->link('catalog/record', $this->data['token_name'].'=' . $this->session->data[$this->data['token_name']] . $url . '&page={page}');
		...
		if (SC_VERSION > 23) {
			$this->template                = 'catalog/record_list';
		} else {
	...
	private function getForm() {
        ...
        if (SC_VERSION > 10) {
			$this->data['upload_images_url'] = $this->url->link('catalog/record/upload_images', 'record_id='.$this->data['record_id'].'&'.$this->data['token_name'].'=' . $this->session->data[$this->data['token_name']]);
		...
		if (!isset($this->request->get['record_id'])) {
			$this->data['action'] = $this->url->link('catalog/record/insert', $this->data['token_name'].'=' . $this->session->data[$this->data['token_name']] . $url);
		} else {
			$this->data['action'] = $this->url->link('catalog/record/update', $this->data['token_name'].'=' . $this->session->data[$this->data['token_name']] . '&record_id=' . $this->request->get['record_id'] . $url);
		}

		$this->data = $this->get_access($this->data);

		$this->data['cancel'] = $this->url->link('catalog/record', $this->data['token_name'].'=' . $this->session->data[$this->data['token_name']] . $url);
		$this->data[$this->data['token_name']]  = $this->session->data[$this->data['token_name']];
        $this->data['keyword'] = '';
        $record_info = array();
		...
		foreach ($this->data['languages'] as $code => $language) {
			if (!isset($language['image']) || SC_VERSION > 21) {
				...
				if (!file_exists(DIR_APPLICATION . $this->data['languages'][$code]['image'])) {
					if (!isset($language['image'])) $language['image'] = $code . ".png";
		...
		if (SC_VERSION > 23) {
			$this->template         = 'catalog/record_form';
		} else {
			...
			try {
		        ...
		        if(in_array($ext, $valid_formats)) {
					...
					if ($size < ($max_size * 1024)) {
	                    ...
	                    $content_upload_file = file_get_contents($this->request->files['file_data']['tmp_name']);
