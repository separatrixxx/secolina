...
class ControllerCatalogLangmark extends Controller
{
	...
	private function save_settings() {
        if ($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			if (!is_array($this->data['ascp_settings'])) $this->data['ascp_settings'] = array();
    ...
    private function load_url_link() {
		...
		$this->data['url_backup'] = str_replace('&amp;', '&', $this->url->link('catalog/langmark/lm_backup', $this->data['token_name'].'=' . $this->session->data[$this->data['token_name']], $this->url_link_ssl));
        $this->data['url_restore'] = str_replace('&amp;', '&', $this->url->link('catalog/langmark/lm_restore', $this->data['token_name'].'=' . $this->session->data[$this->data['token_name']], $this->url_link_ssl));
                    ...
                    if ($this->data['store_id'] == $store['store_id']) {
			            ...
			            $aoptions = Array(
			            	...
			            	'description_status' => true,
			             	...
			             	'ex_multilang_uri' => "=product/search" .PHP_EOL. "#=account/login" .PHP_EOL. "#=account/register" .PHP_EOL. "=journal3/product",
