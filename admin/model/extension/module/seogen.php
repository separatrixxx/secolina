...
class ModelExtensionModuleSeogen extends Model {
	...
	public function urlifyCategory($category_id) {
		...
		if (count($category)) {
			$this->generateCategory($category[0], $this->config->get('module_seogen'));
		}
	}
	...
	public function urlifyManufacturer($manufacturer_id) {
		...
		if (count($manufacturer)) {
			$this->generateManufacturer($manufacturer[0], $this->config->get('module_seogen'));
		}
	}

	public function urlifyInformation($information_id) {
		$information = $this->getInformations($information_id);
		$this->generateInformation($information[0], $this->config->get('module_seogen'));
	}
		...
		if(!empty($data['products_template'])) {
			if(isset($data['products_overwrite'])) {
				...
				if($setOnlyManufacturers || $setOnlyCategories) {
					...
					$this->db->query("DELETE FROM `" . DB_PREFIX . "seo_url` WHERE `query` IN (" . $sql . ")" .
                        " AND language_id=" . (int)$data['language_id']);
				} else {
	...
	public function generateManufacturers($data) {
		if(!empty($data['manufacturers_template'])) {
			if(isset($data['manufacturers_overwrite'])) {
				...
				if($setOnlyManufacturers) {
					$this->db->query("DELETE FROM `" . DB_PREFIX . "seo_url` WHERE `query` IN" .
						" (SELECT CONCAT('manufacturer_id=', manufacturer_id) FROM `" . DB_PREFIX . "manufacturer`" .
                        " WHERE manufacturer_id IN (" . implode(",", $data['only_manufacturers']) . "))" .
                        " AND language_id=" . (int)$data['language_id']);
				} else {
	...
	private function generateProduct($product, $data) {
		...
		if(!empty($data['products_template']) && (isset($data['products_overwrite']) || empty($product['keyword']))) {
			...
			$this->db->query("INSERT INTO `" . DB_PREFIX . "seo_url` SET `query`='product_id=" . (int)$product['product_id'] . "'," .
	...
	private function generateManufacturer($manufacturer, $data) {
		...
		if(!empty($data['manufacturers_template']) && (isset($data['manufacturers_overwrite']) || empty($manufacturer['keyword']))) {
			...
			$this->db->query("INSERT INTO `" . DB_PREFIX . "seo_url` SET `query`='manufacturer_id=" . (int)$manufacturer['manufacturer_id'] . "',".
                " keyword='" . $this->db->escape($keyword) . "'," .
                " language_id=" . (int)$language_id);
		}
	...
	public function generateInformation($information, $data) {
		...
		if(!empty($data['informations_template']) && (isset($data['informations_overwrite']) || empty($information['keyword']))) {
			...
			$this->db->query("INSERT INTO `" . DB_PREFIX . "seo_url` SET `query`='information_id=" . (int)$information['information_id'] . "',".
                " keyword='" . $this->db->escape($keyword) . "'," .
                " language_id=" . (int)$language_id);
		}
	...
	private function getCategories($seogen, $category_id = false) {
		...
		$sql = "SELECT cd.*, u.keyword FROM " . DB_PREFIX . "category_description cd" .
				" LEFT JOIN " . DB_PREFIX . "seo_url u ON (CONCAT('category_id=', cd.category_id) = u.query AND u.language_id=cd.language_id)" .
	...
	private function getProducts($seogen, $product_id = false) {
		...
		if ($query->num_rows == 1) {
            $query_keyword = $this->db->query("SELECT `keyword` FROM `" . DB_PREFIX . "seo_url` WHERE `query`='product_id=" . $query->rows[0]['product_id'] . "'" .
	...
	private function getManufacturers($seogen, $manufacturer_id = false) {
		...
		$sql = "SELECT md.*, u.keyword, m.name, m.manufacturer_id" .
				...
				" LEFT JOIN " . DB_PREFIX . "seo_url u ON (CONCAT('manufacturer_id=', m.manufacturer_id) = u.query AND u.language_id = md.language_id)";
		if($manufacturer_id) {
	...
	private function getInformations($seogen, $information_id = false) {
		...
		$query = $this->db->query("SELECT id.*, u.keyword FROM " . DB_PREFIX . "information_description id" .
								  " LEFT JOIN " . DB_PREFIX . "seo_url u ON (CONCAT('information_id=', id.information_id) = u.query AND u.language_id = id.language_id)" .
