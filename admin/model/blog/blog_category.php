...
class ModelBlogBlogCategory extends Model {
	...
	public function addBlogCategory($data) {
		...
		if (isset($data['blogcategory_seo_url'])) {
			foreach ($data['blogcategory_seo_url'] as $store_id => $language) {
	...
	public function getBlogCategory($blog_category_id) {
		$query = $this->db->query("SELECT DISTINCT * FROM " . DB_PREFIX . "blog_category WHERE 
		blog_category_id = '" . (int)$blog_category_id . "'
		");
	...
	public function getBlogCategories($parent_id, $data = array()) {
		if (!isset($blog_category_data)) {
			$blog_category_data = array();

			if (isset($data['filter_name']) && !empty($data['filter_name'])) {
				$filter = " AND cd.name LIKE '%" . $this->db->escape($data['filter_name']) . "%' COLLATE utf8_general_ci ";
			} else {
				$filter = '';
			}
		
			$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "blog_category c LEFT JOIN " . DB_PREFIX . "blog_category_description cd ON (c.blog_category_id = cd.blog_category_id) WHERE c.parent_id = '" . (int)$parent_id . "' AND cd.language_id = '" . (int)$this->config->get('config_language_id') . "' ".$filter." ORDER BY c.sort_order, cd.name ASC");
	...
	public function getBlogCategorySeoUrls($blog_category_id) {
		$blogcategory_seo_url_data = array();
		
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "seo_url WHERE query = 'blog_category_id=" . (int)$blog_category_id . "'");

		foreach ($query->rows as $result) {
			$blogcategory_seo_url_data[$result['store_id']][$result['language_id']] = $result['keyword'];
		}

		return $blogcategory_seo_url_data;
	}	
