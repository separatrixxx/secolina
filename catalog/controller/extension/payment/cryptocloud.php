...
class ControllerExtensionPaymentCryptocloud extends Controller {
	...
	{
		$this->language->load('extension/payment/cryptocloud');
		$order_id = $this->session->data['order_id'];
		$this->load->model('checkout/order');
		$order_info = $this->model_checkout_order->getOrder($this->session->data['order_id']);

		$merchant_id = $this->config->get('payment_cryptocloud_merchant_id');
		$apikey = $this->config->get('payment_cryptocloud_apikey');
		...
		$this->load->model('checkout/order');
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/extension/payment/cryptocloud')) {
			return $this->load->view($this->config->get('config_template') . '/template/extension/payment/cryptocloud', $data);
		} else {
			return $this->load->view('/extension/payment/cryptocloud', $data);
		}
	}
	...
	public function callback()
	{
		...
		$this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_cryptocloud_order_status_id'), $comment, $notify = true, $override = false);
