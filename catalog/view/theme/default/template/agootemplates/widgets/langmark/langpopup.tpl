    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        foreach ($languages as $language) {
        	if ($language['current']) {
        		echo $settings_widget['title_list_latest'][$language['language_id']];
        	}
        }
        ?></h4>
...
function lm_<?php echo $settings_widget['cmswidget']; ?>_afterload(){
	if (!lm_<?php echo $settings_widget['cmswidget']; ?>_afterLoad_state) {
		document.body.removeEventListener('touchstart', lm_<?php echo $settings_widget['cmswidget']; ?>_afterload); document.body.removeEventListener('touchmove', lm_<?php echo $settings_widget['cmswidget']; ?>_afterload); document.body.removeEventListener('mouseover', lm_<?php echo $settings_widget['cmswidget']; ?>_afterload); document.removeEventListener('mousemove', lm_<?php echo $settings_widget['cmswidget']; ?>_afterload);

        $('#langmarkmodal_<?php echo $settings_widget['cmswidget']; ?>').click();
...
if (/Android|iPhone|iPad|iPod|Windows Phone|webOS|BlackBerry/i.test(lm_<?php echo $settings_widget['cmswidget']; ?>_userAgent)) {
	document.body.addEventListener('touchstart', lm_<?php echo $settings_widget['cmswidget']; ?>_afterload); document.body.addEventListener('touchmove', lm_<?php echo $settings_widget['cmswidget']; ?>_afterload); document.addEventListener('DOMContentLoaded', function() { setTimeout(lm_<?php echo $settings_widget['cmswidget']; ?>_afterload, 7000) });
} else {document.body.addEventListener('mouseover', lm_<?php echo $settings_widget['cmswidget']; ?>_afterload); document.addEventListener('mousemove', lm_<?php echo $settings_widget['cmswidget']; ?>_afterload); document.addEventListener('DOMContentLoaded', function() { setTimeout(lm_<?php echo $settings_widget['cmswidget']; ?>_afterload, 5000); }); }
