...
class ModelBlogBlog extends Model {
	...
	public function getLatestBlogs($data = array()) {
		if (!empty($data['category'])) {
			$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "blog i LEFT JOIN " . DB_PREFIX . "blog_description id ON (i.blog_id = id.blog_id) LEFT JOIN " . DB_PREFIX . "blog_to_store i2s ON (i.blog_id = i2s.blog_id) LEFT JOIN " . DB_PREFIX . "blog_to_category b2c ON (i.blog_id = b2c.blog_id)  WHERE id.language_id = '" . (int)$this->config->get('config_language_id') . "' AND i2s.store_id = '" . (int)$this->config->get('config_store_id') . "' AND i.status = '1' AND i.sort_order <> '-1' AND b2c.blog_category_id IN (".implode(',', $data['category']).") ORDER BY i.sort_order, i.blog_id DESC LIMIT " . (int)$data['start'] . "," . (int)$data['limit'] . "");
