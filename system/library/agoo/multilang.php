...
class agooMultilang extends Controller
{
	...
	public function __construct($registry) {
        ...
        // Fix opencart bug with config_language

        if (!isset($this->request->cookie['language']) && !isset($this->request->cookie['langmark_nocookie'])) {
        	// subdomains
        	/* 16/08/2022
			setcookie('langmark_nocookie', null, -1, '/', $this->host);
        	setcookie('langmark_nocookie', true, time() + 60 * 60 * 24 * 30, '/', $this->request->server['HTTP_HOST']);
			*/
        } else {
        	if (isset($this->request->cookie['langmark_nocookie'])) {
        		unset($this->request->cookie['langmark_nocookie']);
        		// subdomains
        		/* 16/08/2022
				setcookie('langmark_nocookie', null, -1, '/', $this->host);
        		setcookie('langmark_nocookie', null, -1, '/', $this->request->server['HTTP_HOST']);
				*/
        	}
        }
        ...
        */
        ...
        //if (!$ajax) {
	                    ...
	                    if (($this_prefix == $route_prefix) || ($this_prefix == '' && $route_prefix != '')) {
							...
							if ($prefix_len_source == $prefix_url && strlen($prefix_url) > $max_len) {
								...
								if (!$ajax) {
									...
									/*
									setcookie('langmark_multi_name', null, -1, '/', $this->host);
            ...
            /**********************************************************************************/
            if (!empty($multiset) && isset($multiset['name']) && $this->set_multi) {
				/* 16/08/2022
				setcookie('langmark_multi_name', null, -1, '/', $this->host);
				setcookie('langmark_multi_name', $multiset['name'], time() + 60 * 60 * 24 * 30, '/', $this->request->server['HTTP_HOST']);
				*/
			}
            ...
            /*
            $switch_lang = true;
			if (isset($this->request->cookie['lm_prefix']) && urldecode($this->request->cookie['lm_prefix']) != $multi_switch['prefix'] && isset($this->request->server['HTTP_REFERER'])) {

                    $referal = str_ireplace(array('http://', 'https://'), array('', ''), $this->request->server['HTTP_REFERER']);
                    $referal_uri = trim(str_ireplace($this->request->cookie['lm_prefix'], '', $referal), '/');

					$pieces_uri_referal = explode('/', $referal_uri);
					$uri_end_referal = $pieces_uri_referal[count($pieces_uri_referal)-1];


                    $now_url_without = str_ireplace(array('http://', 'https://'), array('', ''), $full_url);
                    $now_uri = trim(str_ireplace($multi_switch['prefix'], '', $now_url_without), '/');

					$pieces_uri_now = explode('/', $now_uri);
					$uri_end_now = $pieces_uri_now[count($pieces_uri_now)-1];

	 				if ($uri_end_referal == $uri_end_now)  {

	 				} else {
                   		$switch_lang = false;
                   		$this->registry->set('lm_switch_lang', true);
	 				}
