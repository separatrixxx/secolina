...
class agooCache extends Controller
{
	...
	public function construct_cache() {
        if (!empty($this->jetcache_settings) && isset($this->jetcache_settings['jetcache_widget_status']) && $this->jetcache_settings['jetcache_widget_status']) {
	...
	public function __call($name, array $params) {
		...
		if ($name == 'delete') {
			if (!empty($this->jetcache_settings) && isset($this->jetcache_settings['jetcache_widget_status']) && $this->jetcache_settings['jetcache_widget_status']) {
				if (isset($this->jetcache_settings['cache_remove_status']) && $this->jetcache_settings['cache_remove_status']) {
					if (isset($params[0]) && is_string($params[0]) && isset($this->jetcache_settings['ex_key']) && $this->jetcache_settings['ex_key'] != '') {
					...
					if ($name == 'set') {
                       ...
                       if (!isset($pieces_array[4])) {
							...
							*/
					        ...
					        $files_key = $this->dir_cache . 'cache.' . preg_replace('/[^A-Z0-9\._-]/i', '', $params[0]) . '.' . (int)$this->is_ssl . '_' . (int)$this->store_id . '_' . (int)$this->language_id . '_' . (int)$this->customer_group_id . '_' . (int)$this->is_mobile . '_' . (int)$this->is_tablet . '_' . (int)$this->is_desktop;
