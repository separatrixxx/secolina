...
class Mfilter_Plus {
			...
			foreach( $val as $val2 ) {
				foreach( $val2 as $val3 ) {
					...
					if( strpos( $val3, '&quot;' ) !== false ) {
						$val4 = md5( str_replace( '&quot;', '"', $this->_trim( mb_strtolower( $val3, 'utf8' ) ) ) );
					...
					if( Mfilter_Helper::i()->isSeoEnabled() && isset( $this->_values['attribute'][$val3.':'.$key] ) && $this->_values['attribute'][$val3.':'.$key] != '-1' ) {
						...
						$val3 = md5( mb_strtolower( $val3, 'utf8' ) );
	...
	public function setValues( $attribs, $options, $filters ) {
				...
				foreach( $val2 as $val3 ) {
					if( is_array( $val3 ) ) {
						foreach( $val3 as $val4 ) {
							...
							$this->_values['attribute'][md5( mb_strtolower( $val4, 'utf8' ) ).':'.$key] = '-1';
						...
						$this->_values['attribute'][md5( mb_strtolower( $val3, 'utf8' ) ).':'.$key] = '-1';
		...
		if( $value ) {
			...
			foreach( $value as $k => $val ) {
				...
				$value[$k] = "'" . md5( $this->_trim( mb_strtolower( $val, 'utf8' ) ) ) . "'";
				...
				if( strpos( $val, '&quot;' ) !== false ) {
					$value[] = "'" . md5( str_replace( '&quot;', '"', $this->_trim( mb_strtolower( $val, 'utf8' ) ) ) ) . "'";
				}
			}
			
			$sql .= '( ';
			$sql .= sprintf( "( `value` IN(%s) AND ( `language_id` = " . (int)$this->config->get('config_language_id') . " OR `language_id` IS NULL ) )", implode( ',', $value ) );
	...
	public function install() {
		...
		if( $this->_addColumn( 'product', 'mfilter_values', 'TEXT NOT NULL DEFAULT ""' ) ) {
			...
			if( version_compare( $version, '1.3.4', '<' ) ) {
				$this->_addColumn( 'mfilter_tags', 'language_id', 'INT(11) NOT NULL' );
			}
			
			if( version_compare( $version, '1.3.6', '<' ) ) {
				try {
					foreach( $this->db->query("SHOW INDEX FROM `" . DB_PREFIX . "mfilter_values`")->rows as $row ) {
						if( $row['Column_name'] == 'type' ) {
							$this->db->query("ALTER TABLE `" . DB_PREFIX . "mfilter_values` DROP INDEX `" . $row['Key_name'] . "`");
						}
					}
	...
	public function installprogress() {
		...
		if( empty( $settings['disable_tags_indexing'] ) ) {
			$steps[] = 'tag';
		}
		...
		switch( $steps[$step] ) {
			case 'attribute' : {
				if( empty( $this->_settings['attribute_separator'] ) ) {
					$sql = "SELECT REPLACE(REPLACE(TRIM(`text`), '\r', ''), '\n', '') AS `val`, `attribute_id` AS `val_id`, `language_id` FROM `" . DB_PREFIX . "product_attribute` GROUP BY `val`, `val_id`";
				} else {
					$sql = "SELECT `text` AS `val`, `attribute_id` AS `val_id`, `language_id` FROM `" . DB_PREFIX . "product_attribute` GROUP BY `val`, `val_id`";
				}
				$rows = $this->db->query("SELECT COUNT(*) AS `c` FROM(SELECT * FROM `" . DB_PREFIX . "product_attribute` GROUP BY `text`, `attribute_id`) AS t");
				$rows = $rows->row['c'];
			...
			case 'tag' : {
				$sql = "SELECT `tag` AS `val`, `language_id` FROM `" . DB_PREFIX . "product_description`";
		...
		if( $sql ) {
			...
			foreach( $this->db->query( $sql )->rows as $row ) {
				...
				foreach( $values as $value ) {
					...
					if( $steps[$step] == 'attribute' ) {
						$val = md5( mb_strtolower( $value, 'utf8' ) );
					...
					if( $steps[$step] == 'tag' ) {
						...
						foreach( $this->seoValues( $seo, $steps[$step], $val_id, isset( $row['language_id'] ) ? $row['language_id'] : null ) as $row2 ) {
							if( $this->db->query(sprintf("
								SELECT 
									* 
								FROM 
		...
		if( $step >= count( $steps ) ) {
			...
			$this->model_setting_setting->editSetting('mfilter_plus_reindexed', array(
				'mfilter_plus_reindexed' => '1'
			));
		} else {
	...
	private function seoValues( $seo, $type, $value_id, $language_id = null ) {
		if( ! in_array( $type, array( 'option', 'filter' ) ) ) {
			return array( array( 'seo' => $seo, 'language_id' => $language_id ) );
		}
	...
	public function uninstall() {
		...
		$this->db->query('DROP TABLE IF EXISTS `' . DB_PREFIX . 'mfilter_tags`');
		
		$this->_removeColumn( 'product', 'mfilter_values' );
		$this->_removeColumn( 'product', 'mfilter_tags' );
		...
		$this->model_setting_setting->deleteSetting( 'mfilter_plus_reindexed' );
	...
	public function updateProduct( $product_id ) {
		...
		$tags			= $this->db->query( "SELECT product_id, tag, language_id FROM `" . DB_PREFIX . "product_description` WHERE `product_id` = " . (int) $product_id );
		$before			= array();
		$after			= array();
		$beforeTag		= array();
		$afterTag		= array();
		...
		if( $attribs->num_rows || $options->num_rows || ( $filters && $filters->num_rows ) || $tags->num_rows ) {		
			...
			foreach( $attribs->rows as $attrib ) {
				...
				foreach( $attrib['text'] as $txt ) {
					...
					$key = $md5 . ':' . $attrib['attribute_id'] . ':' . $attrib['language_id'];
					...
					if( ! isset( $items['attribute'][$key] ) ) {
						...
						$items['attribute'][$key] = array( 
							...
							'gid' => null,
							'language_id' => $attrib['language_id'],
						);
					}
				}
			}
			...
			foreach( $options->rows as $option ) {
				...
				$key = $seo . ':' . $option['option_id'] . ':' . $option['option_value_id'] . ':' . $option['language_id'];
			...
			if( $filters ) {
				foreach( $filters->rows as $filter ) {
					...
					$key = $seo . ':' . $filter['filter_group_id'] . ':' . $filter['filter_id'] . ':' . $filter['language_id'];
			...
			foreach( $tags->rows as $tag ) {
				...
				foreach( $tag['tag'] as $v ) {
					...
					$items['tags'][] = array( 'tag' => mb_strtolower( $v, 'utf8' ), 'language_id' => $tag['language_id'] );
				}
			}
			...
			if( $conditions['tags'] ) {
				$sqlTag[] = '( ' . implode( ' OR ', array_unique( $conditions['tags'] ) ) . ' )';
			}
			...
			if( $sql ) {
				...
				foreach( $this->db->query( $sql )->rows as $row ) {
					$values[$row['type']][($row['type']=='attribute'?$row['value'].':':'').$row['value_id'].':'.$row['language_id']] = $row['mfilter_value_id'];
				}
			}
			...
			if( $sqlTag ) {
				...
				foreach( $this->db->query( $sqlTag )->rows as $row ) {
					$values['tags'][mb_strtolower( $row['tag'], 'utf8' ).':'.$row['language_id']] = $row['mfilter_tag_id'];
				}
			}
						...
						if( ! isset( $values['tags'][$key] ) ) {
							$this->db->query(sprintf("
								...
								SET
									...
									`language_id` = '%s'
								",
								$this->db->escape( $val['tag'] ),
								$val['language_id']
							));
							
							$afterTag[] = (string) $this->db->getLastId();
							$values['tags'][$key] = (string) $this->db->getLastId();
						} else {
							$afterTag[] = $values['tags'][$key];
						}
					} else {
		...
		if( $before ) {
			...
			foreach( $parameters as $parameter ) {
				...
				foreach( $parameter['ids'] as $id ) {
					...
					} else if( ! empty( $values[$id][null]['id'] ) ) {
						$new_ids = array();
